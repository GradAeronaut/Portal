...
function closeMapInfo() {
    ...
    const tooltip = document.getElementById('plateTooltip');
    if (tooltip) {
        tooltip.style.display = 'none';
    }
}

function togglePlateTooltip(event) {
...
function saveMapInfo() {
    ...
    const inscriptionInput = document.getElementById("inputInscription");
    const inscription = inscriptionInput ? inscriptionInput.value : '';
...
// Счётчик символов для личной надписи
document.addEventListener('DOMContentLoaded', function() {
    const input = document.getElementById('inputInscription');
    const counter = document.getElementById('inscriptionCounter');
...
function initMap() {
  ...
  <rect x="35" y="51" width="3" height="22" fill="#000000"/>
    ...
    (async () => {
        ...
        <div style="opacity:.85;">${row.inscription || ''}</div>
      </div>`;
